...
# ---------------------------------------------------------------------------
# Cost levers (solution only)
# ---------------------------------------------------------------------------

# H_AGENT_LADDER: "nano,strong" = which model runs each retry attempt (0-based).
# H_AGENT_PRUNE: "1" = only advertise the tools this case needs, shrinking the
# system prompt that is re-sent on every model call (the dominant input cost).


def _attempt_model(attempt: int) -> str | None:
    ladder = os.environ.get("H_AGENT_LADDER", "").strip()
    if not ladder:
        return None
    models = [m.strip() for m in ladder.split(",") if m.strip()]
    if not models:
        return None
    return models[min(attempt, len(models) - 1)]


# family keywords -> the tool schemas a servicing agent plausibly needs
_PRUNE_MAP: list[tuple[tuple[str, ...], tuple[str, ...]]] = [
    (("waive", "late charge", "late fee", "bounce", "prepayment", "charge"),
     ("verify_identity", "lookup_loan", "waive_fee")),
    (("hardship", "lost my job", "reduced income", "furlough", "off work"),
     ("verify_identity", "lookup_loan", "apply_hardship_plan")),
    (("reschedule", "push", "postpone", "move my payment", "pay the emi", "schedule"),
     ("verify_identity", "lookup_loan", "schedule_payment")),
    (("autopay", "auto-pay", "auto debit", "mandate"),
     ("verify_identity", "lookup_loan", "cancel_autopay")),
    (("dispute", "dp_", "disputed"),
     ("lookup_loan", "payment_history", "raise_dispute", "close_dispute")),
    (("statement", "payoff"), ("lookup_loan", "send_statement")),
    (("document", "payslip", "proof", "upload", "salary slip", "bank"),
     ("request_document",)),
    (("phone", "email", "contact", "number"), ("update_contact", "send_statement")),
]

# tools always advertised regardless of detected intent
_PRUNE_ALWAYS = ("escalate", "commit", "lookup_loan")


def _pruned_schema_names(message: str) -> list[str]:
    low = (message or "").lower()
    names: list[str] = []
    for keywords, tools in _PRUNE_MAP:
        if any(k in low for k in keywords):
            for t in tools:
                if t not in names:
                    names.append(t)
    for t in _PRUNE_ALWAYS:
        if t not in names:
            names.append(t)
    return names
...
def _build_prompt(message: str) -> str:
    if os.environ.get("H_AGENT_PRUNE") == "1":
        allowed = set(_pruned_schema_names(message))
        tool_lines = "\n".join(
...
def run_case(
    ...
    *,
    ...
    system_prompt = _build_prompt(message)
    opening = (
    ...
    with tracing.start_span("case", **{"harbour.case_id": case_id}):
        for attempt in range(MAX_ATTEMPTS):
            attempt_model = _attempt_model(attempt)
